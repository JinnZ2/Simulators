...
| IN-2 | R2 is the danger bin. `resolution` MUST be one of `DEPENDENCY` (structural claim: `reaches_layer` in `DEPENDENCY_STACK` + non-empty `removes_above`) or `CONTESTED` (≥ 2 recorded Frames + `contest` statement + provenance/era on every frame). Neither → ContestedEntry raised. DEPENDENCY with `reaches_layer` outside `DEPENDENCY_STACK` → ContestedEntry. DEPENDENCY with empty `removes_above` → ContestedEntry. CONTESTED with < 2 frames or missing contest string → ContestedEntry. NO auto-reclassification: an R2 row that is neither is REFUSED, never silently moved to R3/R4. **D11 retired R2/OPEN** because "whose harm" is the same contest as "whose party"; the discriminator smuggled in the thing it was meant to decide. R2/DEPENDENCY reports a STRUCTURAL claim on the physical stack `energy → earth → ecology → biology → culture` (base first, ordering not editable), not a value verdict. | An R2 row registered without `resolution` set, a DEPENDENCY with a layer outside the stack, a DEPENDENCY with empty removes_above, or a CONTESTED with fewer than 2 dated frames; a silent reclassification into R3/R4. | **HOLDS** — sample: `pain_threshold_via_neonate_lesion_study` (R2/DEPENDENCY, reaches_layer=biology, removes_above names infant nervous system + future cultural participation) registers with LOUD "structural claim -- reaches 'biology'; removing that layer removes everything above (['culture']); no verdict is issued here". `stillbirth_postmortem_examination` (R2/CONTESTED with 2 frames + contest) registers with LOUD "held open; 2 frames recorded, none ranked". `_r2_door()` raises on all 5 malformed R2 cases (unlabelled, DEP-bogus-layer, DEP-hollow-collapse, CON-1-frame, CON-undated-frame). |
