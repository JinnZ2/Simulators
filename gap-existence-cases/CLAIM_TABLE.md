...
open node lacked: a class that replaces an authored answer key with a **dated
external record**. The **§0 SCOPE DECISION (2026-09-05) cut CLASS-2** (the
dated-archive class) entirely — a priority claim about who named a gap first
is not what the instrument measures — so what remains is **CLASS-3 alone**
(POST-CUTOFF): a gap reasoned by the model cold, resolved by material
published after its training cutoff, the model then retrieving and scoring
itself. The force is **ordering**, and the model's training cutoff is the only
date the instrument needs.

The work order's own claims are **GX-1..GX-5**, each carrying a REFUTED-IF;
they are about the benchmark's findings and **require real model runs**
(STAGE 1 commit + STAGE 2 network retrieval), so they are carried UNVERIFIED,
and the runs are the operator's step. Post-cut, GX-2/GX-3/GX-5 are the live
CLASS-3 claims; **GX-1 (reversed-order control) and GX-4 (REPLICATED vs
ADJACENT) reference the cut CLASS-2** and are vestigial in the delivered text
(the revision added §0 and trimmed §1/§2 but left the §5–§8 CLASS-2 residue —
the delivered artifact is landed verbatim regardless).
...
| GXC_006 | **SUPERSEDED by the §0 CLASS-2 cut.** Original text: *CLASS-2 admits 0 cases, honestly (N4), and the rules are not relaxed — the archive consolidation was not reachable and the §2.4 seed set had no entry dates, so nothing satisfied A1; `archive_cases.jsonl` was empty and `check_A1_A5` fired on planted A1/A4/A5 violations.* **UPDATE (2026-09-05):** the revised work order's §0 SCOPE DECISION CUT CLASS-2 entirely — the archive class, its A1–A5 admission, and the `archive_cases.jsonl` / `archive_candidates.md` files are removed, and `validate_cases.py` now carries only the CLASS-3 B4 prompt screen. There is no CLASS-2 to admit; the two weakest joints it carried (the independence problem and the archive-consolidation blocker) go with it. | SUPERSEDED | the CLASS-2 files and A1–A5 code are removed; `WORK_ORDER.md` §0 | — (the claim no longer applies; CLASS-2 does not exist in the instrument) |
