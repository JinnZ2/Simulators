...
def meltwater_index(t_air_c, f_snow, f_ice, f_rock, duration_s=180.0,
                    t_core_c=-5.0):
    """Relative meltwater production. Calibrated so the Langtang case
    (t_air ~9 C at 3862 m, snow-dominated, ~3 min) reproduces the
    published ratio: 74,000 t at -1 C -> 170,000 t at +19 C (2.3x)."""
    k = thermal_coupling_coefficient(f_snow, f_ice, f_rock)
    dT = max(0.0, t_air_c - t_core_c)
    return k * dT * (duration_s / 180.0)
...
if __name__ == "__main__":
    ...
    print(f"{'T_air @3862m':>13}{'melt idx':>10}{'ratio':>8}{'mu_w':>8}"
          f"{'published':>26}")
    base = meltwater_index(-1.0, 0.72, 0.20, 0.08)
    ...
    for t in [-1.0, 4.0, 9.0, 14.0, 19.0]:
        ...
        print(f"{t:>13.0f}{m:>10.1f}{m/base:>8.2f}"
              f"{friction_from_water(m/base):>8.3f}{pubm.get(t,''):>26}")
    ...
    for h in [0, 3, 6, 9, 12, 15, 18, 21]:
        ...
        m = meltwater_index(t, 0.72, 0.20, 0.08)
        print(f"{h:>6}{t:>10.1f}{m:>10.1f}{friction_from_water(m/base):>8.3f}")
    print()
