...
| `uninstrumented/` | quantities excluded by an instrument's constitution | eight-mechanism register + `scan.py` text scanner, checks on both, and five delivered cases the schema cannot hold — the fifth stating that nothing excludes it, against the register's own opening rule; forty-nine `UNI_*` claims |
