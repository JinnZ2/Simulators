...
| `notes/` | storage for operator entries | mundanely named; entries stored as delivered, checkers never edit them. `operators/D2.md` stated-vs-actual divergence reading, checked against this tree: 5 of 7 instances resolve, its two representations turn out to be five different kinds of pair, its stated signature holds on one instance and fails on four, and its standing check names one of its own two sources |
