...
| `uninstrumented/` | quantities excluded by an instrument's constitution | eight-mechanism register + `scan.py` text scanner, checks on both, eight delivered cases the schema cannot hold, plus a `specimens/` directory and `AVENUES.md`; the drop that attached its primary sources made a delivered reading checkable and one of five did not survive; sixty-eight `UNI_*` claims |
