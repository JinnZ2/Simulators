...
| `uninstrumented/` | quantities excluded by an instrument's constitution | eight-mechanism register + `scan.py` text scanner, checks on both, and four delivered cases the schema cannot hold — the fourth declining to be one entry or two, with its technical claim settled by simulation; forty-one `UNI_*` claims |
