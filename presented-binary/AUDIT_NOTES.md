...
| file | status |
|------|--------|
| `binary_audit.py` | delivered, verbatim |
| `frame_sim.py` | delivered, verbatim |
| `CLAIM_TABLE.md` | delivered, verbatim |
| `cases/` | **claimed in CLAIM_TABLE, did not arrive** |
| fixtures for `frame_sim` | **claimed in CLAIM_TABLE, did not arrive** |
| `presented_binary_audit.py` | added |
| `AUDIT_NOTES.md` | added |
| `samples/`, `.gitignore` | added |

`cases/` is left absent rather than reconstructed. It is data — a framing
someone presented — and inventing one would put words in the author's
mouth. The `frame_sim` properties are different: they are properties of
code, and §2–§3 check all three directly.

Also re-delivered in this drop: `category-weld/CLAIM_TABLE.md` and
`category-weld/welds/capital.json`, both **byte-identical** to the landed
copies. Files that live in one place do not drift (`measurement-fork/`
`MF_019`).
...
|----|-------|--------------|--------|
| PB_001 | Both verification sentences in `CLAIM_TABLE.md` name artifacts the drop does not carry — the seeded case and the fixtures | the files arriving | UNVERIFIED (case), see PB_002–PB_003 (fixtures) |
...
| PB_005 | `--submit3` is parsed but absent from the documented usage, so the documented workflow leaves `dominated_on_own_metric` at `None` — which is the whole of B9 | `--submit3` appearing in the usage block | SUPPORTED |
...
## 1 — PB_001, the verification sentences
...
Neither the case file nor the fixtures are in the drop. `python3
binary_audit.py` exits 1 with *no case files*.

This is the second consecutive drop where the claim table's verification
sentence names a file the drop does not carry. In `category-weld/` the
named files arrived one drop later and `CW_001` closed. The same falsifier
applies here.

The difference is what can be done in the meantime. `category-weld`'s
missing files were code, so they were reconstructed and one reconstruction
was wrong in a way that produced a false finding (`CW_004`). Here the
missing case is *data*, and the three claimed `frame_sim` properties are
code behaviour — so the case stays absent and the properties get checked
directly. Two of the three hold as stated; the third is a naming problem.
