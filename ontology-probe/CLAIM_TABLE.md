...
asserted.
The operator then asked about `optimize` and `optimization`. They
enter under the scope class beside `maximize`, requiring the same four
fields, grounded in the same register entry; `optimal` stays an alias
of the absent `better/worse`, since it claims a position on a gradient
where the verbs name an operation whose objective was dropped. The
fold-matrix register files `optimal` under `optimization`; this file
does not, and the basis says so, because a declared objective names
the gradient's axis and a bare superlative does not. Neither verb is in
run 1's `terms_added`, so both print on the declared-and-unadded line.
