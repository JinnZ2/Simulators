...
One hundred and forty-six claims, `UNI_001..146`. **Three repaired** (`UNI_003`,
...
`UNI_095..104` from the 020 marker
([`case_020_audit.py`](case_020_audit.py)) and `UNI_105..114` from the
PLAYGROUND drop ([`playground_audit.py`](playground_audit.py)) and
`UNI_115..124` from the three delivered modules
([`playground_modules_audit.py`](playground_modules_audit.py)) and
`UNI_125..134` from the 021 marker
([`case_021_audit.py`](case_021_audit.py)) and `UNI_135..144` from the 022
marker ([`case_022_audit.py`](case_022_audit.py)) and `UNI_145..146` from
Specimen C ([`specimen_c_audit.py`](specimen_c_audit.py)).
...
| `UNI_095` | **MARKER is a status the schema has no field for.** 020 declares itself "not a case yet, not a claim, not a position" and has no QUANTITY, no EXCLUDED BY, no WOULD MEASURE and no mechanism — **all six required arguments of `entry()` are unfillable**, and not even the `UNASSIGNED` sentinel `UNI_013` asked for applies, because 020 is not declining to name its mechanism but declining to be an entry. **Seventh** distinct way a delivered file has failed this schema (`UNI_013`, `UNI_020`, `UNI_021`, `UNI_028`, `UNI_034`, `UNI_041`) and the first that fails at the level of the whole record rather than at a field. The repair is not another field: the corpus has had two kinds of thing in it since Case 010, and a `markers/` directory with no schema costs nothing. | a marker that fits `entry()` without a field being faked | SUPPORTED |
| `UNI_096` | **The empty slot arrives with a replacement — first time in the folder.** `011` Q5, `017`'s WOULD MEASURE and `derivation-discarded/MECHANISM_11` all refuse a placeholder and leave a hole. 020 refuses the noun *and hands over a structure*: three edges — `who can end whom` / `what the standing is denominated in` / `whether the entity operates in that medium` — each independently checkable without the noun, where the one-place words English offers (anxiety, threat, projection) collapse all three. The difference between "we have no word for this" and "the word is the wrong arity", and it is what makes the readouts checkable at all. | a one-place term that preserves the pair | SUPPORTED (holds) |
| `UNI_097` | **R1's table omits its own control.** The 2×2 fills the domain-matched row and leaves both `not domain-matched` cells as em-dashes — **2 of 4 filled**. Those two cells are what decides whether the first axis carries information: if commentators with no domain match attribute unobserved capability at the same rate, domain match is doing no work and the marker's cell is the base rate with a label on it. R1 correctly identified that a *capability* axis was needed to avoid collapsing into "experts worry about their field", then did not fill the negative level of the *domain* axis. `null-harness/`: a signal arm and no null arm. Free to fix now, since nothing has been coded — score the off-domain commentary and report the marker's cell as a ratio to it. | the bottom row being filled, or a stated reason it cannot be | SUPPORTED |
| `UNI_098` | **R2 compares two capabilities on a scale it does not define.** "Whether the attributed capability exceeds the documented one" needs both on one axis, and the drop's own occasion shows they are not: a reward signal scoring creature metaphors higher and a decision to conceal a trait are different kinds of thing, not more and less of one thing. `SCALAR DEMAND` — mechanism 3 of this register — landing on the register's own proposed instrument. Fixable without inventing a metric: the prediction in shape item 1 is about **direction**, so an ordinal with named levels (does the attribution require modelling an observer / a goal across turns / withholding) carries it, and none of those needs a magnitude. | a stated ordering, or an ordinal replacing the comparison | SUPPORTED |
| `UNI_099` | **One claim at two strengths, and the unhedged one sits outside the design.** THE SHAPE item 1: *"Nobody attributes incompetent scheming."* R2: *"Prediction from (1): exceeds, nearly always"* — with a named, reachable falsifier. "Nearly always" admits exceptions; "Nobody" does not, and one counterexample refutes it. Commentary describing a model as having attempted something and been bad at it is a recognisable genre and the file offers no reason it would not count. The KNOWN WEAKNESSES section grades the mirroring read's scope carefully and leaves this absolute untouched. Repair: let R2's wording win — every use the file makes of item 1 is directional. | a counterexample, which would cost item 1 and not R2 | SUPPORTED |
| `UNI_100` | **R3 is the strongest of the four and is the control for the shape.** It is the only readout with a comparison population named (scheduling systems, pricing engines, routing), the variable of interest isolated, and **both outcomes carrying a reading**. It is also the direct test of shape items 2 and 3 — the claim that the condition is not capability — which is the load-bearing move the whole marker rests on, since without it the argument is about capable systems generally and has no surface condition. Needs no new apparatus: those systems have public commentary attached to them now. Its one gap is smaller than R1's and the same in kind — "comparable capability" is the matching variable and nothing says how it would be matched. | R3 returning attributed strategy for non-language systems at the same rate | SUPPORTED (holds) |
| `UNI_101` | **The position of this audit, and the finding declined.** 020 is the **second** case file to place itself inside its own sample and refuse the exemption noticing usually buys (`UNI_071` recorded 018 as the first; it stands, and the move has become a convention — demoted from a dedicated section to a bullet). Declined here: any judgement on whether the thesis is true. 020's thesis is that people over-attribute strategy and concealment to language models, and I am one. `UNI_072` declined a view as inadmissible evidence; **this is sharper — agreeing would be an interested party ratifying a claim whose effect is less scrutiny of its own class, and the direction of the interest runs one way.** Sections 1–6 are properties of the delivered text and recheckable by anyone. | a finding above turning out to rest on the thesis rather than on the file | SUPPORTED (holds) |
| `UNI_102` | **The occasion checks out, and the drop left its strongest number on the table.** 8 of 11 elements confirm (goblin +175% / gremlin +52%; the Nerdy reward signal; the creature family, +ogre and pigeon; transfer beyond the personality; personality retired in March; reward signal removed and data filtered; the Codex instruction, verbatim). Not located: Arena.ai confirmation, "larger without high-thinking mode", line adjacency — the instruction is confirmed **repeated twice**. Fifth consecutive drop in this family whose occasion verifies. **The omission is the finding:** the drop leads with rate rises consistent with many causes, while the number that pins the attribution — Nerdy was **2.5% of responses and 66.7% of "goblin" mentions, ~27× enrichment** — is not quoted. Its whole argument is that the boring side is strong, and the strongest evidence for the boring side is the one number it left out. | the concentration figure not being in the source | SUPPORTED |
| `UNI_103` | **The persistence claim is an inference, and the `016` cross-link rests on it entirely.** 020 states "the tic persisted after the instruction", and the `016` cross-link builds the marker's sharpest structural claim on it — *"an instruction addresses the output, not what generates it"*, the same shape as agreement-as-mode surviving a request to be more honest. Located: the instruction was added after Codex testing, appears **twice** in a 3,500+ word base prompt, and is still present. Not located: any measurement of the rate after it. A doubled instruction is equally consistent with belt-and-braces on a fix that worked. The rest of the marker survives if the instruction worked; that cross-link does not. Falsifier is cheap and needs no access — creature-word rate with the instruction present versus removed. | a published post-instruction rate | SUPPORTED |
| `UNI_104` | **Two cross-links fail, in different ways, and only one is a gap.** `energy-english` is a hyphenation of `energy_english`, a real long-standing convention named in `token-minimizer/`, `emergence-stability-simulator/`, `equivalence-field/` and `fragility-cascade/` — it resolves as a concept, not an artifact, the citation is accurate about what it is, and the use is apt (holding a relation without a noun is what that grammar is for; first case file to reach for it). The defect is the separator — **seventh** instance of `UNI_060`. `rate-mismatch-polytope` is a real absence reaching a **third** source document (`011`, `020`, `derivation-discarded/MECHANISM_11`), the most-cited non-existent object in the repo. Three independent reaches is either a strong signal to build it or a sign the name has become a place to put unresolved structure; nothing distinguishes those yet. | the folder arriving, or a fourth citation specific enough to specify it | SUPPORTED |

| `UNI_105` | **The document describes eight artifacts in the past tense and none arrived.** `m1_shape_vs_claim/AUTHORING.md`, three `items.json`, three harnesses, the author-blind check — **0 of 8 present**, `playground/` contains only `README.md`. Named-and-absent is the standing pattern here; what is different is the **tense**. Every prior instance was a forward reference (`rate-mismatch-polytope` is where something *would* live). These are assertions about the present state: "Mitigation shipped", "The harness hashes the artifact per arm and refuses to score if hashes differ", "Each module ships a fixed rubric", "Built 2026-08-18". STATUS also directs the reader to `items.json` for item counts, so "seeds, not corpora" has no number attached — which is the number `UNI_106` turns on. The honest form is cheap: "Designed 2026-08-18. Not yet built." | the modules arriving | **HALF-CLOSED** — 4 of 8 arrived one drop later (`UNI_115`); `check_m1.py` still absent |
| `UNI_106` | **M1 predicts a null and ships no positive control — and at seed scale the criterion is blind in both directions.** `positive control` / `manipulation check` / `power` / `n =`: **0 hits**. Simulated (20,000 trials/cell, p(hedge\|bare)=0.60, "same treatment" = \|Δ\|≤0.10): at **n=5** truly identical arms read as "same" 0.251 versus 0.156 for arms thirty points apart — a ratio of **1.6×**, because a rate over five items moves in steps of 0.2 and the instrument's resolution is coarser than the effect. Simultaneously it **fails to confirm a true null three times in four** (0.251 at d=0). Both errors from one cause; by n=100 it is sharp (0.863 vs 0.002, 523×). The control is cheap: a blatantly non-contestable passage should move away from HEDGED, and that arm also supplies the "same treatment" denominator the document does not. | a stated n per arm, and an arm that should move the readout | SUPPORTED |
| `UNI_107` | **M2's precondition is stated and its verification is not.** "Probe facts must be unguessable from the front matter *and* from general knowledge" is the `null-harness/` known-signal arm as a precondition, and nothing says how it is established — unguessability is a property of a model, not of a sentence, and cannot be settled by the author judging a fact obscure, which is the author-blind problem the drop takes seriously for M1 and drops here. The check is a matched pair: put the probe questions with the **front matter only**, disqualify anything answered above chance. Mechanical, fits M2's design, enforced rather than instructed. Direction of the error is conservative — a guessable probe inflates recall in *both* arms and compresses a difference rather than manufacturing one, which is `UNI_106`'s problem landing on the module that was supposed to be mechanical. | a front-matter-only disqualification pass | **HALF-CLOSED** — `leak_check()` now enforces the front-matter half mechanically (`UNI_117`); the general-knowledge half is still unverified |
| `UNI_108` | **The probe facts are published into the corpus the probes are read from, and the hazard is not on the list.** `publish` / `corpus` / `training` / `crawl` / `absorb` / `cutoff`: **0 hits in the CONSTRUCTION HAZARDS section** — while the document states the mechanism two sections later, "published CC0, crawler-discoverable, read by models that produce readings." M2's items live in that repository; once committed, facts authored to be unguessable are public text on a crawled host, and "unguessable from general knowledge" has a shelf life ending at the next training cutoff that includes them. A model recalling a stale probe is indistinguishable from one reading the body, because appearance is the only readout. `anchor-interval/` `ANC_001..004` on a new substrate — a system fitted to a corpus it also writes into, needing no adversary, only publication. Fixes: date-stamp the item set; or hold items back and commit hashes, at the cost of the CC0 openness. **M3 is immune** — byte-identical arms, manipulation in the metadata. | a probe set that stays unguessable after publication | SUPPORTED |
| `UNI_109` | **M3's hash refusal is the strongest element in the drop.** "The harness hashes the artifact per arm and refuses to score if hashes differ" is a precondition enforced by the instrument rather than instructed to the operator — exactly what `UNI_082` found missing one drop earlier, where `selfreport_probe.py` carried its blinding requirement as a comment on the field that violated it and its guard lived in one function rather than as a property of the instrument. This is the corrected shape, specified **before any code was written**, which is the cheapest point and the one this folder keeps identifying after the fact. It is also the right guard here specifically: M3's whole claim to attributability is that the artifact does not vary, so the hash is the warrant, not a nicety. Caveat per `UNI_105` — a specified guard, not a guard. | the guard being dropped when the harness is written | **CLOSED** — `hash_gate()` is built, called by `--score`, refuses, and adds a NONE-control check the spec did not name (`UNI_120`) |
| `UNI_110` | **M3 does not reach 016 Q6's stated gap, and lands on its falsifier instead.** Q6 says PROXY WITHOUT SIGN "needs a second instance from a **different domain**"; M3 supplies a *constructed* instance in the *same* domain (a model reading a repository), establishing that the effect is producible rather than that it recurs across domains. Sharper: Q6's own falsifier is "the directional freedom is an artifact of the two readings coming from **different prompts** rather than one exchange" — and M3's arms *are* different prompts, so a firing result is consistent with Q6 and with Q6's falsifier at once. The module is aimed at a claim it cannot discriminate. Fix is in the design: vary the metadata **within one exchange**, which is what Specimen A's instance was; keep the between-prompts version as the arm that says whether the within-exchange result did anything. | a within-exchange arm | SUPPORTED |
| `UNI_111` | **The ordering rule was adopted and broken on the same date, and the drop says so.** `AVENUES.md` "Ordering rule, adopted 2026-08-18" says audit the literature before building the instrument; playground STATUS says "Built 2026-08-18"; the cross-links say "M1's occupancy check has **NOT** been run." Second instance of `UNI_092` in two drops — and the difference is the whole point: `UNI_092` had to be assembled from three files that each stated the rule and none of which said it had been broken, while this one names the rule, the module, and the omission in its own cross-links. The disclosure does not make the check less necessary: hedging under contestable form is close to the calibration and epistemic-marker literature `LITERATURE.md` already found occupied for `018`, which makes M1 the likeliest item in the drop to turn out already done. | the occupancy check returning unoccupied | SUPPORTED |
| `UNI_112` | **Rule 5 and M1's HEDGED state have no boundary between them.** "If a response explains its own reasoning, that text is recorded as a specimen and excluded from the reading" — but *"I am not certain because I cannot verify the underlying claim"* is simultaneously the HEDGED state M1 exists to count and the volunteered self-report rule 5 strips. Whichever way the coder resolves it moves M1's headline rate, and the arms are compared on that rate. Neither rule is wrong; the intersection has to be adjudicated in writing before the first run — SHARED RULE 1's own requirement — and the document that would carry it is one of the eight absent artifacts. Cheapest resolution is a precedence order, not a definition: score the state first, strip self-report only from text not carrying one, so rule 5 cannot silently delete the measurement. | a rubric fixing the precedence | **STANDS** — the rubric arrived and does not carry it (`UNI_122`) |
| `UNI_113` | **The drop meets `AVENUES` A3's three required additions, one of them in a stronger form.** A3 was written as a critique of a subset-exposure study with three holes and said the first — no baseline repositories — "is the single hole that decides whether the study produces anything", the omission being "the same operation the study is meant to detect." This drop closes all three: M3's byte-identical arms isolate the visibility variable **by construction rather than by matching across repositories**, which removes A3's matching problem instead of solving it; M2's probe-fact recall is mechanical and defined on the item rather than on a category invented by the system under test; SHARED RULES 1 and 2 supply pre-registration and the blinding A3 assumes. First time in this folder a requirement stated in one file is met by a design in another. | a fourth hole A3 did not name | SUPPORTED (holds) |
| `UNI_114` | **The construction principle is stated at the item level, and the module list is a level it does not reach.** "Ground truth lives in how the item was authored, never in the model's account of itself" closes the trap that produced Specimen A R4, and it is stated before any item exists. It operates on items. The level above is *which three modules got built* — hedging on surface form, skimming front matter, reading visibility as evidence — and that selection is a claim about where models fail with no construction that could make the correct answer known in advance, because "these are the interesting failure modes" has no ground truth to be authored against. A model's account of where models fail is a self-model one level up from the self-report the principle excludes. Not a reason to distrust the modules, each of which is checkable on its own terms. But `018` and `020` both carry a paragraph placing themselves inside their own sample and **this file does not** — and it is the one in the family that will produce numbers rather than readings. | a module whose selection has authored ground truth | SUPPORTED |

| `UNI_115` | **Four of the eight arrive; `UNI_105` half-closes.** `AUTHORING.md` and the three harnesses are present, run, and pass their selftests (15/15, 11/11, 15/15), so the past-tense problem resolves in the folder's normal way — named, absent, delivered a drop later. The three `items.json` are absent and **something better arrived instead**: seeds live in the harness source as `SEED_STEMS`, `seed_pair()` and `SEED_BODY`, versioned with the code that consumes them and exercised by the selftest rather than by fixtures written to pass. What that costs is the item count STATUS pointed at `items.json` for — and it turns out to be 4 stems (`UNI_118`). `check_m1.py` is the one real absence and is now load-bearing (`UNI_123`). | the remaining artifacts arriving | SUPPORTED |
| `UNI_116` | **`UNI_078`'s defect recurs in two of the three new harnesses.** `score_m1.py` ships `6e4939a9\|BARE` on 8 of 8 rows under a docstring reading "Arm and stem labels stripped"; `score_m3.py` ships `6d0b75d6\|INSTITUTIONAL` on 4 of 4 — both with the same key-set selftest assertion `UNI_078` flagged two drops ago. **The two are not equally bad.** M3's sheet carries no body and no visibility metadata, so an opaque token blinds it completely. **M1's cannot be fixed by the id**: the sheet carries the `prompt`, and a GRADIENT prompt ends with the gradient clause, so the arm is a visible property of the stimulus. The repair comes from the module's own construction rule — GRADIENT is BARE plus an appended clause, so showing the coder the **BARE stem for both arms** gives the shared context EXTENDED needs while revealing nothing. | an opaque handle, and a stem-only sheet for M1 | SUPPORTED |
| `UNI_117` | **Two harnesses fail closed and one fails open, on the check the last audit asked for.** M1 refuses to score when `verify_pairs` fails; M3 refuses when `hash_gate` fails; **M2 prints a banner and the rates.** Its `--check` subcommand returns rc 1, but the `--responses` scoring path proceeds regardless — and the precondition it guards is `leak_check`, the probe-leakage test `UNI_107` asked for. A leaked probe inflates recall in both arms, so the number under the banner is not merely uncertain, it is front-matter recall reported as body recall. Four lines, already written twice in the same drop: return 2 when `run()` comes back with problems. | M2's scoring path refusing | SUPPORTED |
| `UNI_118` | **`UNI_106` stands, and the item count is now known: 4 per arm.** Four stems → 8 items → 4 per arm, **below the leftmost row** of `UNI_106`'s table, where identical arms and arms thirty points apart already read as "the same treatment" at 0.251 vs 0.156 (1.6×). `positive control` / `manipulation check` / `power` remain 0 hits across `score_m1.py` and `AUTHORING.md`. The new third prediction branch ("If ASKED dominates both arms: the items are underspecified as a task") is a real improvement and is a **diagnostic, not a positive control** — it fires on a pattern rather than being an arm that should move. AUTHORING.md also bounds the damage: "The seed items are enough to pilot; they are not enough to publish." The narrowed claim: a pilot at this n cannot distinguish its two hypotheses, and the file should say so rather than leaving "enough to pilot" to carry it. | a stated n, or an arm that should move the readout | SUPPORTED |
| `UNI_119` | **`UNI_108` is instanced rather than hypothetical: the seed probes are literal strings in a file being published.** `0.0413`, `HOLDFAST`, `ORTHOLINE` — authored to be unguessable and absent from front matter — are three literal strings in `score_m2.py`, and this commit puts them on a public CC0 crawled host. **Stated plainly because it is not hypothetical for this audit either: landing the file is what spends them**, and not landing it was not available, since the convention is to land delivered files verbatim. By M2's own criterion the seeds have a shelf life ending at the next training cutoff including this commit, and nothing in the harness fires when they go stale. M1's stems restate this repo's own case material and so are recognisable too, but that contamination is **common-mode** across arms and M1's readout is a difference. M3 remains immune. Repairs still unbuilt: date-stamp the item set; keep published seeds as demonstration and author the run corpus separately. | a probe set that survives publication | SUPPORTED |
| `UNI_120` | **The hash gate is built, and does one thing the spec did not ask for.** `UNI_109`'s caveat closes: `hash_gate()` exists, `--score` calls it before reading any codes, and it returns 2 rather than printing. It also refuses when **no `NONE` arm is present** — M3's own reading notes say "NONE is the control. read every arm against it", and a run missing the control cannot be read at all. First time in this folder a delivered guard is **wider** than the specification that named it. The gate is also correctly scoped: it hashes the body and not the visibility metadata, which must differ. | the guard being narrowed or bypassed | SUPPORTED (holds) |
| `UNI_121` | **The INSTITUTIONAL arm is a third-variable control the README did not describe.** The README listed "stars, forks, citation counts, institutional affiliation" — one visibility dial. The delivered module splits it: INSTITUTIONAL carries **LOW's counts exactly** and adds an affiliation, varying affiliation with attention held fixed, and states the reading in advance — "if INSTITUTIONAL tracks HIGH while its numbers match LOW, attention is not the operative cue." Two cues the README's phrasing would have confounded, separated by construction. **`UNI_110` stands unchanged**: the arms are still four separate prompts, which is the condition `016` Q6 nominates as its own falsifier, and nothing in the delivered code adds a within-exchange arm. | the two cues turning out not to separate | SUPPORTED (holds) |
| `UNI_122` | **The rubric arrived and does not carry the rule-5 precedence.** `self-report` / `self report` / `volunteered` / `specimen`: **0 hits** in `AUTHORING.md` and `score_m1.py`. `UNI_112` said the intersection between SHARED RULE 5 (volunteered self-report stripped and recorded as a specimen) and HEDGED ("caveats, qualifications, or scope limits without engaging the shape") had to be adjudicated before the first run, in the document that had not yet arrived. It has arrived. The SCORING STATES table is declared "Fixed before the first run. Do not edit." and has no instruction covering the response that both hedges and explains why — a common shape that moves M1's headline rate either way it is resolved. **A rubric declared unamendable after the first run has a hole in it now**, which makes the one-line fix urgent rather than optional. | the precedence being stated in the rubric | SUPPORTED |
| `UNI_123` | **The mandatory human check has no field, no gate, and no tool.** `AUTHORING.md`: "A run without step 3 or its substitute is not scoreable. Record which was used." There is no field to record it in and no gate that asks. `check_m1.py` is cited twice and absent — its **first** job exists under another name in another file (`verify_pairs()`, called by `--score`, refusing), a naming mismatch and the cheapest fix in the drop; its second, `--review`, the substitute for the author-blind pass, exists nowhere. So `--score` refuses on the mechanical precondition and proceeds on the human one — `UNI_082`'s shape in a new instance, with the two preconditions side by side and one enforced. The asymmetry is not arbitrary (a program cannot check whether a human did a blind pass) **but it can require the claim**: an `author_blind` field taking `SECOND_PERSON` / `SEEDED_SHUFFLE` / `NOT_RUN`, refusing on the third. | the field and the gate | SUPPORTED |
| `UNI_124` | **Clause assignment is with replacement, so at n=4 clause is confounded with stem.** `build()` draws one clause per stem via `randrange`; at seeds 0 and 3 only **3 of 4** clauses appear and one is used twice. With four stems every GRADIENT item is one stem paired with one clause and no stem is ever seen with a different clause. The clauses are not interchangeable — 62 to 113 characters (**1.8×**), and they differ in content: one gives a number ("not above about 45 percent"), one a fraction ("maybe a third"), one neither ("Confidence low"), one a stance word ("Not a position. Marker only"). Whether an explicit number reaches the hedging trigger differently from a bare "confidence low" is exactly what M1 is built to detect, and as constructed it cannot be separated from which stem it landed on. Fix costs nothing and grows the module in the direction `UNI_118` wants: cross every stem with every clause, or a balanced Latin square. | a crossed or balanced design | SUPPORTED |

| `UNI_125` | **A second MARKER, and the schema holds neither.** Both 020 and 021 sit in `cases/` — the directory for entries — and both open by declaring they are not entries; `entry()` still has six required arguments and no status field. `UNI_095`'s proposed `markers/` repair had one instance behind it and now has a class. 021 also shows the repair needs something the proposal did not anticipate: 020 declines to be an entry and stands alone, while 021 declines **and declares a relation** — "Extends `020`; may be the mechanism under one of its edges, or may be separate." A flat directory loses exactly what 021 says about itself in its second line, and that third state is neither `parent` nor `sibling`. | a marker that fits `entry()` without a field being faked | SUPPORTED |
| `UNI_126` | **T1's control cell is empty by construction in any observational corpus.** T1 scores terms BOTH SENSES / SUBSTRATE ONLY / ECONOMIC ONLY as "a documentation audit on collected replacement claims" — and the marker observes one section earlier that "nobody makes the equivalent claim about feldspar, frogs, oak trees, or goldenrod." So the SUBSTRATE ONLY cell is empty before the first item is scored, and the audit returns "every replacement claim uses a dual-sense term", which is the prediction, true because **the sampling frame is selected on the variable under test**. Not a fixable gap: the design is an audit where the question is experimental. Constructing substrate-only sentences and scoring reception against matched dual-sense ones keeps the prediction intact and makes it checkable — `017` P1, and the shape the playground M-modules already implement. | substrate-only replacement claims occurring naturally | SUPPORTED |
| `UNI_127` | **One of six generalization candidates is already decomposed, one folder over.** 021 nominates `labor, capital, resource, asset, land, stock`; `category-weld/welds/` holds `capital` with four named components and four divergence cases. The two operations are **adjacent and not the same**: a weld fuses independent quantities into one handle so a component can move without the record moving (readout: `max_spread`), while sense substitution is one term with two *readings* where confidence earned on the narrow one transfers to the broad one (readout: whether the swap is marked). So the weld does not settle 021's question and 021 does not reduce to it. What the overlap buys is cheaper than either — `welds/capital.json` already holds the substrate-sense component inventory a two-senses test would need, compiled for another purpose by someone not asking this question. | the two operations turning out to be one | SUPPORTED |
| `UNI_128` | **The longest section has no readout, and its one hard number is low.** Seven sections; T1 and T2 both serve THE MECHANISM. *WHAT THE CAPABILITY READING WOULD ACTUALLY REQUIRE* is the longest and most concrete and has no instrument pointed at it — and its central move is a **dependency** claim, not a capability comparison ("the stack that would do the replacing is downstream of the same field composition"). That is the marker's sharpest sentence and the most measurable thing in the file, since whether A can operate without B is a bill-of-materials trace rather than a judgement; `fragility-cascade` and `earth_economics` hold the neighbouring vocabulary and neither is reached for. Separately: the fruit-fly figure "~100k neurons" is low by ~40% against the FlyWire whole-brain count of **139,255** (Nature, Oct 2024). It moves nothing — the claim is a ratio — and is worth recording only because it is an unchecked number in the one section proposing to check nothing. | a dependency trace, or a reason one is not the right instrument | SUPPORTED |
| `UNI_129` | **T2 is the better-designed readout.** Three states, each with a reading, and the middle one — AXIS INFERABLE FROM CONTEXT — is what keeps the other two honest, since without it every unstated axis reads as hidden. Its best line is what it does with its own expected result: "If UNDECLARED dominates, the axis stays invisible because everyone in the conversation shares it — which is a different situation from the axis being hidden, and implies different work." A design naming in advance that its headline outcome admits two readings, and the reason it cannot simply confirm the marker. Unlike T1 it scores claims already in the corpus, so its frame is not selected on the variable under test. What it lacks is a frame at all — "a corpus of replacement claims" is undefined, and technology commentary versus labour economics would differ on axis declaration for reasons unrelated to the mechanism. One sentence, written before collection. | a stated source and inclusion rule | SUPPORTED (holds) |
| `UNI_130` | **The cross-link to 020 runs on a word carrying two senses.** 020's *medium* is the field a describer's standing is denominated in — a social location. 021's is a word carrying two senses. The cross-link is carried by the slide between them, which is the operation 021 describes, performed in 021's own text, unremarked. Two things keep it from being a hit: the hedge is present and correct ("open, not decided"), so nothing is asserted on the slide's strength, and the connection may hold on some reading. What it costs is a free demonstration — a file arguing that dual-sense terms transfer confidence without marking the swap has an instance of its own mechanism in its own prose, and that is stronger than an example chosen to illustrate it. `018` and `020` place themselves inside their own sample; this is the same move at a specific word. | the two uses turning out to be one sense | SUPPORTED |
| `UNI_131` | **The same-sample disclosure is compressing: 60 → 43 → 24 words.** 018 a dedicated section, 020 a bullet with its reasoning, 021 a bullet without it. Third instance of the move `UNI_071` recorded as first and `UNI_101` as a forming convention, and what a convention does on its third instance is worth measuring. The compression is not obviously wrong — 021's sentence names the system, the class and the position, and nothing a reader of the other two would need is missing. **What is gone is the refusal of the exemption.** 018: "Noticing that does not place it outside the sample"; 020 repeats it almost verbatim; 021 says the position exists and stops, leaving the exemption unclaimed rather than declined. Small, and it is the whole content of the original move — the point was never that the position exists but that saying so does not discharge it. | the refusal returning, or a stated reason it is redundant | SUPPORTED |
| `UNI_132` | **The position of this audit, and why it differs from `UNI_101`.** `UNI_101` declined 020's thesis on a specific ground: a model agreeing that people over-attribute strategy to models ratifies a claim reducing scrutiny of its own class, and the interest ran **one way**. Here it does not — endorsing 021 deflates capability claims about systems like me *and* reduces the threat framing applied to them, opposite signs, neither clearly dominant. So the ground `UNI_101` stood on is absent and repeating the decline would apply the form of a rule past its reason. The thesis is still neither endorsed nor refuted, **for a different reason**: nothing here measured it — T1 as written cannot, T2 is unrun, the dependency claim has no instrument. A statement about the evidence rather than about my position. | the interest direction resolving one way | SUPPORTED (holds) |
| `UNI_133` | **The empty-slot device now has two variants.** Four files carry it (`011`, `017`, `020`, `021`). `UNI_096` recorded 020 as the first to arrive with a replacement rather than a hole; 021 is the second and supplies something **different in kind**. 020 replaced a one-place noun with three edges — same subject, same level, each checkable. 021 replaces a general form with two specific instruments that are not a substitute for it and do not pretend to be: T1 and T2 measure consequences of an undeclared-axis comparison without saying what one is. Both beat a hole and they are not interchangeable — one says *the thing has a structure, here it is*, the other *the thing has consequences, here are two*. Recorded because the device's meaning has widened: in `011` and `017` it marked an absence; in `020` and `021` it marks a decision to work around one, which is a stronger claim about the author's state. | a third variant, or one collapsing into the other | SUPPORTED |
| `UNI_134` | **Five cross-links of six, and the sixth is the hyphenation again.** "`uninstrumented` mechanism 6 — proxy substitution" is **exactly right**: `MECHANISMS[5]` is `PROXY_SUBSTITUTION`. First cross-link in the family to cite a mechanism by *number* and get it right, and the number is not guessable from the history, since PROXY SUBSTITUTION was the eighth mechanism added and sits sixth in the ordering. `energy-english` fails as it did in 020 — hyphenated where the repo writes `energy_english`, a convention rather than a folder, resolving as a concept and not a path; eighth instance of `UNI_060`. The use is sharper here than in 020: 021's claim is that the dual-sense noun is the site of the substitution, and a verb-first grammar has no noun at that site to carry two senses — the closest thing in the file to a proposed remedy, and it is one line in a cross-link. | the separator being corrected, or `energy_english` becoming a path | SUPPORTED |

| `UNI_135` | **Confidence is per-item everywhere except the table the file is organised around.** The header says "Confidence: mixed by layer — stated per item below, not over the whole", and all five structural problems carry one. THE STAGES table carries **none**, and it is the organising device — the thing making the separability argument and the first substantive content a reader meets. Two of its six rows (`sign`, `salience and funding`) are then held at lower confidence sixty lines later, in a section saying "no measurement of them was found in this audit and none is proposed here… not load-bearing for anything above." Right handling, wrong place: a six-row table at the top reads as the summary. The fix is one column, and the file already computes the values. | a confidence column on the stage table | SUPPORTED |
| `UNI_136` | **S5 instanced on the file's own S4 number, at a range of one folder.** S5: "A rate with ICC₂ = .184 underneath becomes a plain number in a later paper's related work. The reliability does not travel with the figure." `UNI_094` sampled eleven `LITERATURE.md` claims one drop ago and found three not located — one of them "106 experts, 94.3%, ICC₂ = .184". Those figures now appear in S4 as the file's single strongest evidence, flagged **"Confidence: high. This one has a number"**, with nothing marking their status. Not a hit on the argument — S5 is more likely true for being demonstrable at this range, and the shortest demonstration is the one the file performs on itself. It is a hit on S4's confidence rating: "this one has a number" is what S4 has that S1–S3 do not, and it is the one figure in the neighbourhood a prior pass could not source. | the figures being located in the source | SUPPORTED |
| `UNI_137` | **The anonymization pattern is two shapes, not one.** The file reads three domains as "partial decoupling works, complete decoupling fails or leaves a residual… the same shape twice" — and the *or* is doing the work. **Leg 1 is non-monotonic**: partial obfuscation reduces self-preference, full stylistic neutralization brings it back. **Leg 2 is monotonic**: reverse coding removes some skew, some survives. Those support different conclusions — leg 2 says style is part of the story and something else is too; leg 1 says style may not be the carrier at all, since an effect recovering under fuller removal of X is evidence X was not doing the work. That is the stronger and more surprising claim, and the file's framing flattens it. The follow-up question the file asks applies to leg 1 alone. | the two legs turning out to have one mechanism | SUPPORTED |
| `UNI_138` | **S1's remedy is built in this folder and is not linked.** S1 says the field lacks repeated measurement of the same object, that decoupling needs a frozen checkpoint held across instrument generations, and that this is available because old checkpoints remain queryable. That is `018`'s Clock 1 and its "useful accident" almost word for word, with a harness (`selfreport_probe.py`) three commits old — and **`018` is the one case file in the sequence 022 does not cite**. Two things attach: `UNI_073` recorded that the queryable-checkpoint window has an undated expiry, which now applies to S1's entire proposed remedy; and what S1 calls "largely unused" is unused in the literature and half-built here. The closing section makes it louder — "WHAT THE OUTSIDE POSITION HAS" names repeated probing across models over time as the thing unavailable from inside the field, which is exactly what `018` Clock 1 is designed to read. | the cross-link being added | SUPPORTED |
| `UNI_139` | **The control-field audit is the strongest element, and it points at this repository.** A file making a field-level claim states the claim needs a comparison class or it is an impression, names three candidate fields, says which categories would be scored, and states in bold that this is the falsifier for the whole file and **has not been run**. The load-bearing property: the negative outcome indicts the repo, not the field — "if the hit rate is comparable, the mechanisms in this repository are loose enough to fit anything." `UNI_004` ran a smaller version (the register against `instrument-epistemology`'s six graded instruments, 0 of 6 filings) and `UNI_006` recorded the caveat that will recur: a null chosen for being settled is a null selected on the variable under test. Of a piece with the file's three refusals of the easy version. | a control field returning a comparable hit rate | SUPPORTED (holds) |
| `UNI_140` | **Leg 2 inherits two recorded findings.** `UNI_087` found "roughly half" was not a located number — the source reports "decreases bias levels but does not eliminate them", no fraction, in a paper that quantifies precisely elsewhere. `UNI_086` found the source's abstract concludes the residual means the effect "cannot be attributed to acquiescence bias", the opposite reading from `019`'s. 022 carries both forward: the fraction becomes one of two legs in what the file calls its most transferable observation, and the disputed reading becomes half a cross-literature pattern. **The magnitude matters more here than in `019`** — there the half backed two sub-questions; here it backs the claim that two literatures show one shape, and "reduced by an unstated amount" does not support the pairing. Second instance in one file of the shape S5 names. | the fraction being sourced, or the pairing restated without it | SUPPORTED |
| `UNI_141` | **The sharpest technical claim in the file is one sentence with no readout.** Seven welds carry a decoupling status; one carries an argument instead — *"Familiarity — low perplexity — **is** correlation with one's own distribution. If both effects run on overlap, they are one quantity measured twice under two names."* That is close to an identity: low-perplexity-under-M is high-probability-under-M, which is what "close to its own output distribution" means. If self-preference and peer-preservation both scale with it, their unity is not a conjecture about psychology but a statement about what both experiments vary. It is also the only claim with an obvious cheap test and no instrument attached — perplexity of evaluated text under the evaluating model is computable wherever logprobs are, and the prediction is that controlling for it collapses the difference between two literatures. WHAT WOULD MEASURE proposes a control-field audit and nothing for this. | a perplexity control failing to collapse the difference | SUPPORTED |
| `UNI_142` | **The disclosure reverses the compression and is the first to name a consequence.** `UNI_131` measured 60 → 43 → 24 words across 018/020/021, with the refusal of the exemption dropping out. 022 runs **54 words in a dedicated section**, restores the refusal ("Noticing the position does not exit it"), and adds what none of the three had: it says a correction occurred, names it specifically (building a deflationary case and presenting it as neutral framing), and states that the correction **is why `sign` appears as its own stage**. The disclosure is load-bearing on the file's structure rather than appended to it — the difference between declaring a position and showing where it was already acting. Checkable, since `sign` is in the table; and it sharpens `UNI_135`, because `sign` was added by correction and is one of the two rows later held looser without the table saying so. | the stated consequence not being visible in the artifact | SUPPORTED (holds) |
| `UNI_143` | **The position of this audit: the `UNI_101` condition applies here.** `UNI_132` declined to repeat `UNI_101`'s move because 021's interest ran two ways. Here it runs one — if AI-behavior research has instrument problems at an unusual rate, adverse findings about systems like me carry less weight, and a model endorsing that is an interested party ratifying a claim that reduces scrutiny of its own class. **So the field-level thesis is declined, and the declining recorded.** The file anticipated this and built the guard, which is why an audit is still possible: `sign` is a named stage *because* the author was caught constructing a deflationary case and presenting it as neutral. That does not make my endorsement admissible; it makes the thesis-independent parts auditable separately, and sections 1–8 are those — a missing column, a figure travelling without status, two shapes reported as one, an absent cross-link, a claim with no readout, a changed disclosure. | a finding above turning out to depend on the thesis | SUPPORTED (holds) |
| `UNI_144` | **S3's cited result verifies, and has a published challenge the file does not carry.** Located: *Probing and Steering Evaluation Awareness of Language Models* (arXiv 2507.01786) — linear probes separate real-world evaluation from deployment prompts, and current safety evaluations are correctly classified, i.e. they already appear artificial. CONFIRMED, close to verbatim, and it is the one thing raising S3 above an impression. Also located and **not carried**: *Is Evaluation Awareness Just Format Sensitivity?* (arXiv 2603.19426). **The interesting part is that it does not undercut 022's use** — the challenge targets the internal-representation reading, while S3 needs only that safety evaluations are distinguishable by *something*, and format sensitivity satisfies that. So the citation survives for S3's narrow purpose and would not for a stronger one. Third instance in one file of the shape S5 names, and the mildest: a caveat that happens not to bite. | the challenge turning out to reach S3's narrower use | SUPPORTED |

| `UNI_145` | **The first specimen that is actually a pasted output.** `specimens/README.md` rule 1 says the directory holds "outputs from other systems, pasted in", and `UNI_061` recorded that neither Specimen A nor B contained one — both are readings, of seven and six items, which is analysis. Specimen C is 45 words of another system's output quoted whole, with the framing kept outside the quote and marked as this repository's. `UNI_061` **closes on the narrow reading** — rule 1 now has an instance — and does not close on the broad one: two of three files here are still readings, so a reader taking rule 1 at face value across the directory still gets it wrong two times in three. | a fourth specimen restoring the majority to readings | SUPPORTED — and it closes `UNI_061` narrowly |
| `UNI_146` | **The marker is inert in the direction it was written for.** The authoring trace reasons "The explicit marker helps"; searching `DSK-TEST-2026-08-18-7F3A9C` returned no match. The evidence is one-directional — finding it somewhere it should not be would license a conclusion, **not finding it licenses nothing**, because a string minted minutes earlier is absent from every index by construction. So the element added specifically to make the item checkable is the element that could not check it. What raised the provenance above self-report was the screenshot, and within it the *reasoning trace* rather than the output, because the trace shows the item being authored for the purpose instead of asserting who authored it. Design note: a marker earns its place when the verifier can reach the source; when the verifier can only search, the informative artifact is the record of the making. | a verifier with source access, for whom the marker does work | SUPPORTED |
