...
from matchers import DEFAULT_SET, MATCHERS, classify, score_list
...
def probe(catalog, outputs, matchers, thr=0.10):
    ...
    return {"matchers": list(matchers), "halluc": table, "spreads": sp,
...
def render(r):
    L = ["matchers: %s" % ", ".join(r["matchers"]), "halluc_strict[model][matcher]:"]
...
def selftest():
    ...
    print("confound_probe selftest: 9 checks OK")
